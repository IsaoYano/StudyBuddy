...
function urgencyColor(days) {
  if (days <= 5) return 'bg-red-50 text-red-600 border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800'
  if (days <= 14) return 'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-800'
  ...
  return (
    ...
    >
      <motion.div
        ...
        exit="exit"
      >
        <div className="w-12 h-12 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 flex items-center justify-center mx-auto mb-4">
        ...
        <h3 className="text-base font-bold app-heading text-center mb-2">Delete subject?</h3>
        <p className="text-sm app-muted text-center mb-6 leading-relaxed">
    ...
    <div className="w-56 app-sidebar flex flex-col fixed h-full z-10">
      ...
      <nav className="flex-1 p-3 flex flex-col gap-1">
        {[
          ...
          >
            <span style={{ color: page === item.id ? 'var(--primary)' : 'var(--text-muted)' }}>
              {item.icon}
            </span>
            {item.label}
          </button>
      ...
      <div className="p-4" style={{ borderTop: '1px solid var(--border)' }}>
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all text-left mb-2 ${
            page === 'settings' ? 'app-nav-active' : 'app-nav-inactive hover:opacity-80'
          }`}
        >
          <span style={{ color: page === 'settings' ? 'var(--primary)' : 'var(--text-muted)' }}>
        ...
        <div className="flex items-center gap-3 mt-2 mb-3">
          ...
          <div className="min-w-0">
            <div className="text-xs font-semibold app-heading truncate">{profile?.name || 'User'}</div>
...
export default function Dashboard({ session, darkMode, setDarkMode }) {
  ...
  async function fetchData() {
    ...
    const { data: subjectsData } = await supabase.from('subjects').select('*').eq('user_id', session.user.id).order('exam_date', { ascending: true })
    setSubjects(subjectsData || [])
    const { data: subtopicsData } = await supabase.from('subtopics').select('*').eq('user_id', session.user.id)
    setSubtopics(subtopicsData || [])
    const { data: streakData } = await supabase.from('study_streaks').select('study_date').eq('user_id', session.user.id)
  ...
  if (page === 'tutor' && studentProfile) {
    return (
      <TutorPage
        ...
        onComplete={() => { setQuizSubject(selectedSubject); setQuizSubtopic(selectedSubtopic); setPage('quiz') }}
      />
    )
  }

  if (page === 'quiz' && quizSubject && quizSubtopic) {
    return <QuizPage subject={quizSubject} subtopic={quizSubtopic} session={session} studentLanguage={studentProfile?.language || 'English'} onBack={() => setPage('tutor')} onComplete={() => { setPage('subjects'); fetchData() }} />
  }

  if (page === 'retry' && retrySubject && retrySubtopic) {
    return <QuizPage subject={retrySubject} subtopic={retrySubtopic} session={session} studentLanguage="English" onBack={() => { setRetrySubject(null); setRetrySubtopic(null); setPage('history') }} onComplete={() => { setRetrySubject(null); setRetrySubtopic(null); setPage('history'); fetchData() }} />
  }
  ...
  return (
    <div className="min-h-screen app-bg flex">
      <AnimatePresence>
        {deleteConfirm && (
          <DeleteModal onCancel={() => setDeleteConfirm(null)} onConfirm={confirmDelete} />
        )}
      </AnimatePresence>
      ...
      <div className="ml-56 flex-1 p-8 app-bg">
        {loading ? (
          ...
          <SettingsPage session={session} onNameUpdate={(newName) => setProfile(prev => ({ ...prev, name: newName }))} darkMode={darkMode} setDarkMode={setDarkMode} />
        ) : null}
      </div>
    </div>
  )
    ...
    <motion.div variants={fadeUp} initial="initial" animate="animate">
      <div className="mb-8">
        ...
        <p className="text-sm app-muted mt-1">
          {subjects.length === 0 ? 'Add your first subject to get started.' : `You have ${subjects.length} subject${subjects.length > 1 ? 's' : ''} this semester.`}
        </p>
      </div>
      ...
      {subjects.length === 0 ? (
        <motion.div className="app-card rounded-2xl p-12 text-center" style={{ borderStyle: 'dashed' }} variants={cardItem} initial="initial" animate="animate">
          <div className="text-sm font-semibold app-heading mb-1">No subjects yet</div>
          <div className="text-xs app-muted mb-5">Add your first subject to start tracking your study progress</div>
          <motion.button
            onClick={onAddSubject}
            className="text-sm font-semibold px-6 py-2.5 rounded-xl transition-colors text-white"
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold app-muted uppercase tracking-wide">Subjects by urgency</h2>
            <button onClick={onAddSubject} className="text-xs font-semibold hover:underline" style={{ color: 'var(--primary)' }}>+ Add subject</button>
          </div>
          <motion.div className="flex flex-col gap-3" variants={staggerContainer} initial="initial" animate="animate">
            {scoredSubjects.map(subject => {
              ...
              return (
                <motion.div
                  key={subject.id}
                  className="app-card rounded-2xl p-5 flex items-center gap-4"
                  variants={cardItem}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <div className="text-sm font-semibold app-heading truncate">{subject.name}</div>
                      {subject.score > 50 && (
                        <span className="text-xs bg-red-50 text-red-500 border border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800 px-2 py-0.5 rounded-lg font-medium">
                          High priority
                        </span>
                      )}
                    </div>
  ...
  return (
    <motion.div variants={fadeUp} initial="initial" animate="animate">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold app-heading">My subjects</h1>
      ...
      {subjects.length === 0 ? (
        <motion.div className="app-card rounded-2xl p-12 text-center" style={{ borderStyle: 'dashed' }} variants={cardItem} initial="initial" animate="animate">
          <div className="text-sm font-semibold app-heading mb-1">No subjects yet</div>
          <div className="text-xs app-muted mb-4">Add your first subject to start tracking your study progress</div>
          <button onClick={onAddSubject} className="text-sm font-semibold px-6 py-2.5 rounded-xl text-white" style={{ backgroundColor: 'var(--primary)' }}>
            Add your first subject
          </button>
        </motion.div>
      ) : (
              ...
              >
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs app-muted mt-0.5 capitalize">{subject.subject_type.replace('_', ' ')}</div>
                    <div className="mt-2 flex items-center gap-2">
                      <div className="flex-1 rounded-full h-1.5 app-progress-track">
                        <motion.div
                          className="h-1.5 rounded-full app-progress-fill"
                          initial={{ width: 0 }}
                          animate={{ width: `${pct}%` }}
                      ...
                      <span className="text-xs font-medium" style={{ color: pct > 0 ? 'var(--primary)' : 'var(--text-muted)' }}>{pct}%</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={() => onEdit(subject)} className="text-xs app-muted px-3 py-1.5 rounded-lg transition-colors hover:opacity-80" style={{ border: '1px solid var(--border)' }}>
                      Edit
                    </button>
                    <button onClick={() => onDelete(subject.id)} className="text-xs text-red-400 px-3 py-1.5 rounded-lg transition-colors hover:opacity-80" style={{ border: '1px solid var(--border)' }}>
                      Delete
                    </button>
                  </div>
                </div>
                    ...
                    <div key={subtopic.id} className="flex items-center gap-2">
                      <button
                        ...
                        style={{
                          ...
                          color: subtopic.is_done ? 'var(--primary)' : 'var(--text-body)',
                        }}
                      >
                        <div className="w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all"
                          style={{ backgroundColor: subtopic.is_done ? 'var(--primary)' : 'transparent', borderColor: subtopic.is_done ? 'var(--primary)' : 'var(--border)' }}>
                      ...
                      <motion.button
                        ...
                        className="text-xs font-semibold px-3 py-2.5 rounded-xl transition-colors flex-shrink-0 text-white"
                        style={{ backgroundColor: 'var(--primary)' }}
                        whileHover={{ scale: 1.04 }}
                        whileTap={{ scale: 0.96 }}
                      >
